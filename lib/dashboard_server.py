#!/usr/bin/env python3
"""
ludics dashboard server with lazy JSON regeneration.

Usage: python3 dashboard_server.py <port> <dashboard_dir> <ludics_bin> [ttl]

Serves static files from dashboard_dir. When data/*.json is requested,
checks staleness and regenerates (via ludics dashboard generate) if
...
DEFAULT_TTL = 5
DATA_FILES = {"slots.json", "ready.json", "notifications.json", "mag.json"}
...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    def __init__(self, *args, ludics_bin="", ttl=DEFAULT_TTL, **kwargs):
        self.ludics_bin = ludics_bin
        self.ttl = ttl
        ...
        if _regen_lock.acquire(blocking=False):
            try:
                if time.time() - _last_regen_time < self.ttl:
                    return
                subprocess.run(
                    [self.ludics_bin, "dashboard", "generate"],
                    timeout=30,
                    capture_output=True,
                )
...
def main():
    if len(sys.argv) < 4:
        print(
            "Usage: dashboard_server.py <port> <dashboard_dir> <ludics_bin> [ttl]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    ludics_bin = sys.argv[3]
    ttl = int(sys.argv[4]) if len(sys.argv) > 4 else DEFAULT_TTL

    os.chdir(dashboard_dir)

    handler = functools.partial(DashboardHandler, ludics_bin=ludics_bin, ttl=ttl)
    server = http.server.HTTPServer(("", port), handler)
    print(f"ludics dashboard serving on port {port} (TTL={ttl}s)", flush=True)
    try:
