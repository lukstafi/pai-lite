...
tasks_file_path() {
  ...
  echo "$(ludics_state_harness_dir)/tasks.yaml"
}
    ...
    while IFS='|' read -r rule_action rule_paths_csv; do
      ...
      for watch_path in "${watch_paths[@]}"; do
        ...
        if [[ ! -f "$expanded_path" ]]; then
          ludics_warn "watch path not found: $watch_path"
          continue
        fi
        local path_id
...
tasks_list() {
  local file
  file="$(tasks_file_path)"
  [[ -f "$file" ]] || ludics_die "tasks file not found: $file (run: ludics tasks sync)"
  awk '
    /^[[:space:]]*-[[:space:]]*id:/ {
      id=$0
      sub(/.*id:[[:space:]]*/, "", id)
...
tasks_convert() {
  ...
  [[ -f "$yaml_file" ]] || ludics_die "tasks.yaml not found (run: ludics tasks sync first)"
...
tasks_write_file() {
  ...
  if [[ -f "$file" ]]; then
    ludics_info "skipping existing: $id"
    return
  fi
  ...
  if [[ "$source" == "watch" && "$id" =~ ^watch-(.+)-[0-9a-f]{8}$ ]]; then
    ...
    if [[ -n "$old_file" ]]; then
      ...
      ludics_info "migrated: $(basename "$old_file" .md) -> $id"
      return
    fi
  fi
...
tasks_files_list() {
  ...
  if [[ ! -d "$tasks_dir" ]]; then
    echo "No task files yet (run: ludics tasks convert)"
    return
  fi
...
EOF
  ...
  echo ""
  echo "Test with:"
  echo "  ludics flow ready      # Should show task-001, task-005"
  echo "  ludics flow blocked    # Should show task-002, task-003"
  echo "  ludics flow critical   # Should show deadline + stalled task"
  echo "  ludics flow impact task-001  # Should show task-002, task-003"
}
...
tasks_queue_elaborations() {
  ...
  while IFS= read -r task_id; do
    ...
    ludics_queue_request "elaborate" "\"task\":\"$task_id\"" >/dev/null
    count=$((count + 1))
  done <<< "$needs_elab"

  if [[ $count -gt 0 ]]; then
    ludics_info "Queued $count task(s) for elaboration"
  fi
}
...
tasks_check_elaboration() {
  ...
  if [[ ! -f "$file" ]]; then
    ludics_die "task file not found: $file"
  fi
...
tasks_migrate_refs() {
  ...
  [[ -d "$tasks_dir" ]] || ludics_die "tasks dir not found: $tasks_dir"
  ...
  for old_file in "$tasks_dir"/watch-*-*.md; do
    ...
    if [[ -f "$new_file" ]]; then
      rm "$new_file"
      ludics_info "removed template: $new_id"
    fi
    ...
    ludics_info "migrated: $old_id -> $new_id"
  ...
  for f in "$tasks_dir"/watch-*-*.md; do
    ...
    if grep -q '^\- \[ \] TBD$' "$f" 2>/dev/null; then
      rm "$f"
      ludics_info "removed orphan template: $fid"
...
tasks_merge() {
  ...
  [[ -n "$target_id" ]] || ludics_die "target task ID required"
  [[ ${#source_ids[@]} -gt 0 ]] || ludics_die "at least one source task ID required"
  ...
  if [[ ! -f "$target_file" ]]; then
    ludics_die "target task not found: $target_id"
  fi

  for src_id in "${source_ids[@]}"; do
    local src_file="$tasks_dir/${src_id}.md"
    if [[ ! -f "$src_file" ]]; then
      ludics_die "source task not found: $src_id"
    fi
    if [[ "$src_id" == "$target_id" ]]; then
      ludics_die "cannot merge a task into itself: $src_id"
    fi
  done
...
tasks_duplicates() {
  ...
  for fp_file in "$tmpdir"/*; do
    ...
    if [[ "$count" -ge 2 ]]; then
      ...
      echo "  -> ludics tasks merge $first_id $other_ids"
      echo ""
    fi
  done
